...
const PROJECTS = [
  ...
  },
  {
    ...
    tags: [TAGS.js, TAGS.node, TAGS.beApp, TAGS.feApp, TAGS.game],
  ...
  },
  {
    ...
    tags: [TAGS.c_sh, TAGS.unity, TAGS.game],
  ...
  },
  {
    symbol: "🦠",
    title: "CoDash",
    tags: [TAGS.js, TAGS.ts, TAGS.react, TAGS.saga, TAGS.redux, TAGS.bootstrap],
  ...
  },
  {
    symbol: "👨🏼‍💻",
    title: "morstad.eu",
    tags: [TAGS.react, TAGS.js, TAGS.ts, TAGS.feApp, TAGS.next, TAGS.tw],
...
export default function Portfolio() {
  ...
  const renderTiles = (tags: TagsT[]) => {
    return PROJECTS.filter((el) =>
      ...
      return (
        <div key={i} className="break-inside w-full mb-4">
          <article className="overflow-hidden rounded-lg shadow-lg dark:shadow-none border border-white">
            ...
            <div className="flex flex-wrap p-2 md:p-4">
              {project.tags.map((tagCode, i) => {
                const tag = tags.find((el) => el.code === tagCode) as TagsT;
  ...
  useEffect(() => {
    const tags = Object.entries(_TAGS).map((e) => {
  ...
  return (
    <div className="flex flex-col justify-between h-screen">
      ...
      <main className="pt-5 pb-5 mx-auto flex-grow flex flex-col max-w-screen-2xl">
        ...
        <div>{mappedTags && renderTags(mappedTags)}</div>
