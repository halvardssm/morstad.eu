...
summary: "Offloading a hot code path to compiled Rust via WASM and FFI."
---
...
3. Write the CPU-intensive function in any language able to compile into [WASM](https://docs.deno.com/runtime/reference/wasm/) and import this into Deno.
4. Write your code in any language that compiles to C and use Deno's built-in [FFI (Foreign Function Interface)](https://docs.deno.com/runtime/fundamentals/ffi/) to call the method

Say that the requirements for the application or script are that it is built using JS and that we do not want to rely on APIs as we want to keep the code contained or offline. We are then left with options 3 and 4. In this post, I show off both the WASM way and the FFI way. The language we will use is Rust, as it is a rather large hype-train amongst developers nowadays. But enough for the introduction, let's get to it!

> ⚠️ For this tutorial, I will assume that you have [Deno](https://docs.deno.com/runtime/getting_started/installation/) and [Rust](https://www.rust-lang.org/learn/get-started) installed on your machine, if not, follow the respective guides to set it up.
...
> You can in practice publish this module to NPM, but I would recommend setting the build target to `bundler` instead of `web` for this use-case and also following a more specific `Node.js` tutorial for that.
...
So, assuming your excitement and anticipation are through the roof, we will now run `make run`. You should get an output similar to this.
